...
/**
 * Carries a {@code char} local variable of a target method into an
 * {@link de.splatgames.aether.weaver.api.Inject @Inject} handler, and the handler's writes back
 * out.
 *
 * <p>A Java parameter is passed by value, so a handler that assigns to a plain {@code char}
 * parameter changes its own copy and leaves the target holding the old one. Declaring the
 * parameter as a {@code LocalCharRef} and the capture as
 ...
 * target local declared {@code char}, and only that. Resolution never widens a primitive: a
 * variable declared {@code int} needs {@link LocalIntRef} and one declared {@code short} needs
 * {@link LocalShortRef}, even though all three occupy an {@code int}-sized slot and Java itself
 * widens a {@code char} without a cast.
 *
 * <p>Which {@code char} is meant is settled by the elements of
 ...
 * {@code AW1054}, and {@code mutable = true} on a plain {@code char} parameter is reported as
 * {@code AW1053} — accepting it would emit code that does nothing and says nothing. A handler that
 * only reads the variable declares a plain {@code char} parameter instead.
 *
 ...
 *
 * <p>A {@code char} holds one UTF-16 code unit. A supplementary code point does not fit in one,
 * so a target that iterates a string by {@code char} has a surrogate half in the variable, and the
 * carrier hands that half over as it is.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 * @Weave(CsvReader.class)
 * public final class DelimiterWeave {
 *
 *     // char separator is a local of parse(String), live where split() is called.
 *     @Inject(method = "parse(java.lang.String)",
 *             at = @At(value = Point.INVOKE, target = "#split"),
 *             require = 1)
 *     private static void beforeSplit(String line,
 *                                     @Local(name = "separator", mutable = true)
 *                                     LocalCharRef separator) {
 *         if (line.indexOf('\t') >= 0) {
 *             separator.set('\t');   // parse() splits on tabs instead
 *         }
 *     }
 * }
 * }</pre>
 ...
 * @see LocalRef
 */
public final class LocalCharRef {
     ...
     */
    public LocalCharRef(final char value) {
        this.value = value;
    }
    ...
    /**
     * Returns a description of the value held, as {@code LocalCharRef[x]}.
     *
     * <p>The character is written as itself and is not quoted or escaped, so a carrier holding a
     * control character or an unpaired surrogate produces exactly that character between the
     * brackets.
     *
    ...
    public String toString() {
        return "LocalCharRef[" + this.value + ']';
    }
}
