...
/**
 * Carries a {@code boolean} local variable of a target method into an
 * {@link de.splatgames.aether.weaver.api.Inject @Inject} handler, and the handler's writes back
 * out.
 *
 * <p>A Java parameter is passed by value, so a handler that assigns to a plain {@code boolean}
 * parameter changes its own copy and leaves the target holding the old one. Declaring the
 * parameter as a {@code LocalBooleanRef} and the capture as
 ...
 * target local declared {@code boolean}, and only that. Resolution never widens a primitive: an
 * {@code int} variable needs {@link LocalIntRef} and a {@code byte} needs {@link LocalByteRef}.
 *
 * <p>Which {@code boolean} is meant is settled by the elements of
 ...
 * {@code AW1054}, and {@code mutable = true} on a plain {@code boolean} parameter is reported as
 * {@code AW1053} — accepting it would emit code that does nothing and says nothing. A handler that
 * only reads the variable declares a plain {@code boolean} parameter instead.
 *
 ...
 * <pre>{@code
 * @Weave(Session.class)
 * public final class GuardWeave {
 *
 *     // boolean valid is a local of authenticate(String), live where audit() is called.
 *     @Inject(method = "authenticate(java.lang.String)",
 *             at = @At(value = Point.INVOKE, target = "#audit"),
 *             require = 1)
 *     private static void beforeAudit(String user,
 *                                     @Local(name = "valid", mutable = true)
 *                                     LocalBooleanRef valid) {
 *         if (Accounts.isService(user)) {
 *             valid.set(true);   // authenticate() goes on with true
 *         }
 *     }
 * }
 * }</pre>
 ...
 * @see LocalRef
 */
public final class LocalBooleanRef {
     ...
     */
    public LocalBooleanRef(final boolean value) {
        this.value = value;
    }
    ...
    /**
     * Returns a description of the value held, as {@code LocalBooleanRef[true]}.
     *
    ...
    public String toString() {
        return "LocalBooleanRef[" + this.value + ']';
    }
}
