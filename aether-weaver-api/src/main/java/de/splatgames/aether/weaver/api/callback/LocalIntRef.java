...
/**
 * Carries an {@code int} local variable of a target method into a handler and the handler's writes back out.
 *
 * <p>A Java parameter is passed by value, so a handler that assigns to an {@code int} parameter changes its own
 ...
 *
 * <p>A carrier is matched by what it holds rather than by its own type, so this parameter resolves against an
 * {@code int} variable of the target. It matches no other primitive: {@code long}, {@code short}, {@code byte},
 * {@code char}, {@code boolean}, {@code float} and {@code double} each have their own carrier, and a variable of
 * one of those is reported as {@code AW1050} rather than widened into this one. A reference variable needs
 * {@link LocalRef}.
 *
 ...
 * handler that stores the carrier somewhere and calls {@link #set(int)} later changes nothing in the target: the
 * target's slot has already been written and the carrier is no longer consulted.
 *
 ...
 *             require = 1)
 *     private void beforePost(@Local(name = "total", mutable = true) LocalIntRef total) {
 *         total.set(total.get() + 1);   // charge() goes on with the incremented total
 *     }
 * }
 * }</pre>
 *
 ...
 * @see LocalRef
 */
public final class LocalIntRef {
     ...
     */
    public LocalIntRef(final int value) {
        this.value = value;
    }
    ...
    /**
     * Returns a description of this carrier and the value it holds, in the form {@code LocalIntRef[7]}.
     *
