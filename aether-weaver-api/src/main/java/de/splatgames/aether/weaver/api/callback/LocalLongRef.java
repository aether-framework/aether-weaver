...
/**
 * Carries a {@code long} local variable of a target method into a handler and the handler's writes back out.
 *
 * <p>A Java parameter is passed by value, so a handler that assigns to a {@code long} parameter changes its own
 ...
 * reads {@link #get()} back into the target's own slot as soon as the handler returns. A {@code long} occupies
 * two local variable slots, and the write-back stores across both of them.
 *
 ...
 * {@code long} variable of the target. It matches no other primitive: an {@code int} variable needs
 * {@link LocalIntRef}, and a variable of a primitive this carrier does not hold is reported as {@code AW1050}
 * rather than converted into it. A reference variable needs {@link LocalRef}.
 *
 ...
 * handler that stores the carrier somewhere and calls {@link #set(long)} later changes nothing in the target:
 ...
 * volatile, so a carrier a handler publishes to another thread carries no visibility guarantee, and a
 * {@code long} field is not guaranteed to be read and written atomically.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 ...
 *             require = 1)
 *     private void beforePost(@Local(name = "cents", mutable = true) LocalLongRef cents) {
 *         cents.set(cents.get() + 1L);   // settle() goes on with the incremented value
 *     }
 * }
 * }</pre>
 *
 ...
 * @see LocalRef
 */
public final class LocalLongRef {
     ...
     */
    public LocalLongRef(final long value) {
        this.value = value;
    }
    ...
    /**
     * Returns a description of this carrier and the value it holds, in the form {@code LocalLongRef[7]}.
     *
    ...
    public String toString() {
        return "LocalLongRef[" + this.value + ']';
    }
}
