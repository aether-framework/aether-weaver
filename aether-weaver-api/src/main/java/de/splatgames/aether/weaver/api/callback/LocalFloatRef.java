...
/**
 * Carries a {@code float} local variable of a target method into an
 * {@link de.splatgames.aether.weaver.api.Inject @Inject} handler, and the handler's writes back
 * out.
 *
 * <p>A Java parameter is passed by value, so a handler that assigns to a plain {@code float}
 * parameter changes its own copy and leaves the target holding the old one. Declaring the
 * parameter as a {@code LocalFloatRef} and the capture as
 ...
 * target local declared {@code float}, and only that. Resolution never widens a primitive: a
 * variable declared {@code double} needs {@link LocalDoubleRef} and one declared {@code int} needs
 * {@link LocalIntRef}, however freely Java itself would convert between them.
 *
 * <p>Which {@code float} is meant is settled by the elements of
 ...
 * {@code AW1054}, and {@code mutable = true} on a plain {@code float} parameter is reported as
 * {@code AW1053} — accepting it would emit code that does nothing and says nothing. A handler that
 * only reads the variable declares a plain {@code float} parameter instead.
 *
 ...
 *
 * <p>{@link #set(float)} takes a {@code float}, so an expression the handler computed in
 * {@code double} arithmetic needs the narrowing cast Java requires anywhere else. The value is
 * stored exactly as given, including {@link Float#NaN} and negative zero; the carrier compares
 * nothing and rejects nothing.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
 * @Weave(Renderer.class)
 * public final class ScaleWeave {
 *
 *     // float scale is a local of draw(Frame), live where blit() is called.
 *     @Inject(method = "draw(com.acme.Frame)",
 *             at = @At(value = Point.INVOKE, target = "#blit"),
 *             require = 1)
 *     private static void beforeBlit(Frame frame,
 *                                    @Local(name = "scale", mutable = true)
 *                                    LocalFloatRef scale) {
 *         scale.set(Math.min(scale.get(), 2.0f));   // draw() blits at the capped scale
 *     }
 * }
 * }</pre>
 *
 ...
 * @see LocalRef
 */
public final class LocalFloatRef {
     ...
     */
    public LocalFloatRef(final float value) {
        this.value = value;
    }
    ...
    /**
     * Returns a description of the value held, as {@code LocalFloatRef[2.0]}.
     *
     * <p>The value is rendered by {@link Float#toString(float)}, so a whole number appears with
    ...
    public String toString() {
        return "LocalFloatRef[" + this.value + ']';
    }
}
