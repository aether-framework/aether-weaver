---
name: java-migration
description: Guide for upgrading Java projects between major versions (8→11→17→21→25). Use when user says "upgrade
Java", "migrate to Java 25", "update Java version", or when modernizing legacy projects.
---
...
strip();        // better trim() (Unicode-aware)
"line1\nline2".
...
of("a","b","c");     // immutable list
Set.

of(1,2,3);            // immutable set
Map.

of("k1","v1");         // immutable map
...
println(s.length());  // s already cast
        }
...
addFirst("first");    // new method
list.

addLast("last");      // new method
list.
...
try(
StructuredTaskScope.ShutdownOnFailure scope = StructuredTaskScope.open()){
