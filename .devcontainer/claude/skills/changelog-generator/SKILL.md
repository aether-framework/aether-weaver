...
description: Generate changelogs from git commits. Use when user says "generate changelog", "update changelog", "what
changed since last release", or before preparing a new release.
---
...
Note: pf4j uses Fixed → Changed → Added → Removed. Keep a Changelog uses Added → Changed → Deprecated → Removed →
Fixed → Security.
