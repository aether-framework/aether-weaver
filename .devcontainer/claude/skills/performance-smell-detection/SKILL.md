...
description: Detect potential code-level performance smells in Java - streams, collections, boxing, regex, object
creation. Provides awareness, not absolutes - always measure before optimizing. For JPA/database performance, use
jpa-patterns instead.
---
...
This skill helps you **notice** potential performance smells, not blindly "fix" them. Modern JVMs (Java 21/25) are
highly optimized. Always:
...
Since **Java 9** (JEP 280), string concatenation with `+` uses `invokedynamic`, not StringBuilder. The JVM optimizes
simple concatenation well.
...
debug("Processing {} with id {}",name, id);
```
...
matches("\\d{3}-\\d{4}")){  // Compiles regex!
...
contains(r.getId())){}  // O(n) each time
        }
...
callExternalApi(request));  // Blocks OS thread
        }
...
UserProfile(user.resultNow(),orders.
